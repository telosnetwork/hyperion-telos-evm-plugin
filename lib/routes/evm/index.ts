...
import {Api, ApiInterfaces} from 'enf-eosjs';
...
import {HyperionAction} from "../../../../../src/interfaces/hyperion-action.js";
import {TelosEvmConfig} from "../../interfaces.js";
import DebugLogger from "../../debugLogging/index.js";
...
export default async function (fastify: FastifyInstance, opts: TelosEvmConfig) {
    ...
    const authorityProvider: ApiInterfaces.AuthorityProvider = {
        getRequiredKeys: (args: ApiInterfaces.AuthorityProviderArgs): Promise<string[]> => {
    ...
    async function getSignature(telosTrxId: string): Promise<any> {
        ...
        try {
            const results = await fastify.elastic.search<HyperionAction>({
            ...
            return results?.hits?.hits[0]?._source.signatures[0];
        } catch (e) {
            console.log(e);
            return null;
        }
    ...
    */
    ...
    async function getCurrentBlockNumber(indexed: boolean = false) {
        if (!indexed) {
            ...
            let currentBlockNumber = "0x" + (Number(results?.hits?.hits[0]?._source["@global"].block_num) - ACTION_BLOCK_LAG).toString(16);
    ...
    methods.set('eth_getLogs', async ([parameters]) => {
        ...
        try {
            ...
            const searchResults = await fastify.elastic.search<HyperionAction>({
                index: `${fastify.manager.chain}-action-*`,
                size: 2000,
                query: queryBody,
    ...
    */
    ...
    async function doRpcMethod(jsonRpcRequest: any, clientInfo, reply: any) {
        ...
        if (methods.has(method)) {
            ...
            try {
                ...
                console.log(`RPCERROR: ${new Date().toISOString()} - ${JSON.stringify({
                    error,
                    exception: e
                })} | Method: ${method} | REQ: ${JSON.stringify(params)}`);
                return {jsonrpc, id, error};
            }
        } else {
    ...
    fastify.post('/evm', {schema}, async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        const usage = parseInt(reply.getHeader('x-ratelimit-remaining') as string);
