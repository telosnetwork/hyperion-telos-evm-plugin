...
async function default_1(fastify, opts) {
    ...
    */
    ...
    async function getReceiptsByTerm(term, value) {
        const termStruct = {};
    ...
    async function hasTopics(topics, topicsFilter) {
    ...
    methods.set('eth_getTransactionReceipt', async ([trxHash]) => {
        if (trxHash) {
            ...
            return {
                ...
                logs: buildLogsObject(receipt['logs'], _blockHash, _blockNum, receipt['hash'], numToHex(receipt['trx_index'])),
    ...
    methods.set('eth_getLogs', async ([parameters]) => {
        ...
        let fromBlock = parseInt(await toBlockNumber(params.fromBlock), 16);
        let toBlock = parseInt(await toBlockNumber(params.toBlock), 16);
    ...
    methods.set('trace_filter', async ([parameters]) => {
        ...
        for (const param_obj of params) {
            ...
            let fromBlock = parseInt(await toBlockNumber(param_obj.fromBlock), 16);
            let toBlock = parseInt(await toBlockNumber(param_obj.toBlock), 16);
    ...
    fastify.post('/evm', { schema }, async (request, reply) => {
